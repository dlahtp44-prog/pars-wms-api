...
async def upload_excel(file: UploadFile = File(...)):
    try:
        ...
        column_map = {'창고':'warehouse','로케이션':'location','품번':'item_code','품명':'item_name','LOT':'lot_no','규격':'spec','수량':'qty'}
        df = df.rename(columns=column_map)
        for _, r in df.iterrows():
            db.add_inventory(str(r.get('warehouse','A')), str(r['location']), str(r['item_code']), 
                             str(r['item_name']), str(r['lot_no']), str(r.get('spec','-')), float(r['qty']), "엑셀 업로드")
